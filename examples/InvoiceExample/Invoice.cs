    ...
    public static void GenerateInvoice()
    {
        ...
        while (idx < items.Count)
        {
            ...
            doc.Page(612, 792, (p) =>
            {
                ...
                p.Text("INVOICE", 55, headerY + 10, 24, new TinyPdf.TextOptions(Color: "#fff", Font: TinyPdf.PdfFont.Times));
                p.Text("#INV-2025-001", 472, headerY + 12, 12, new TinyPdf.TextOptions(Color: "#fff"));

                if (isFirstPage)
                {
                    // Company & billing info (first page only)
                    p.Text("Acme Corporation", margin, 670, 16);
                    p.Text("123 Business Street", margin, 652, 11, new TinyPdf.TextOptions(Color: "#666"));
                    p.Text("New York, NY 10001", margin, 638, 11, new TinyPdf.TextOptions(Color: "#666"));

                    p.Text("Bill To:", 340, 670, 12, new TinyPdf.TextOptions(Color: "#666"));
                    p.Text("John Smith", 340, 652, 14);
                    p.Text("456 Customer Ave", 340, 636, 11, new TinyPdf.TextOptions(Color: "#666"));
                    p.Text("Los Angeles, CA 90001", 340, 622, 11, new TinyPdf.TextOptions(Color: "#666"));
                }
                else
                {
                    // Indicate continuation on subsequent pages
                    p.Text("Invoice (continued)", margin, headerY - 10, 10, new TinyPdf.TextOptions(Color: "#fff"));
                }
                ...
                p.Text("Qty", 300, tableHeaderY + 8, 11, new TinyPdf.TextOptions(Align: "right", Width: 40));
                p.Text("Price", 350, tableHeaderY + 8, 11, new TinyPdf.TextOptions(Align: "right", Width: 90));
                p.Text("Total", 450, tableHeaderY + 8, 11, new TinyPdf.TextOptions(Align: "right", Width: 112));
                ...
                while (idx < items.Count && y - rowHeight >= footerLimit)
                {
                    ...
                    p.Text(qty, 300, y, 11, new TinyPdf.TextOptions(Align: "right", Width: 40));
                    p.Text(price, 350, y, 11, new TinyPdf.TextOptions(Align: "right", Width: 90));
                    p.Text(total, 450, y, 11, new TinyPdf.TextOptions(Align: "right", Width: 112));
                ...
                if (idx >= items.Count)
                {
                    ...
                    p.Text("$7,000.00", 450, y - 15, 11, new TinyPdf.TextOptions(Align: "right", Width: 112));
                    p.Text("Tax (8%):", 350, y - 35, 11);
                    p.Text("$560.00", 450, y - 35, 11, new TinyPdf.TextOptions(Align: "right", Width: 112));
                    p.Rect(370, y - 65, pw + margin - 370, 25, "#2563eb");
                    p.Text("Total Due:", 380, y - 53, 12, new TinyPdf.TextOptions(Color: "#fff", Font: TinyPdf.PdfFont.Courier));
                    p.Text("$7,560.00", 450, y - 53, 12, new TinyPdf.TextOptions(Color: "#fff", Align: "right", Width: 112));

                    // Footer
                    p.Text("Thank you for your business!", margin, 80, 12, new TinyPdf.TextOptions(Align: "center", Width: pw, Color: "#666"));
                    p.Text("Payment due within 30 days", margin, 62, 10, new TinyPdf.TextOptions(Align: "center", Width: pw, Color: "#999"));
                }
                else
                {
                    // On non-final pages, show a small footer note
                    p.Text("Continued on next page", margin, 80, 10, new TinyPdf.TextOptions(Align: "center", Width: pw, Color: "#999"));
                }
            });
        }
