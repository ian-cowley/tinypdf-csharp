...
public static class Report
{
    public static byte[] GenerateReport(int iteration = 0, bool writeToFile = true)
    {
        ...
        doc.Page(612, 792, (p) =>
        {
            ...
            p.Text("Executive Summary", margin, 690, 12, new TinyPdfCreate.TextOptions(Font: TinyPdfCreate.PdfFont.Courier));
            p.Text("This quarter saw strong growth across our core metrics. Revenue increased by 12% year-over-year.", margin, 672, 10, new TinyPdfCreate.TextOptions(Width: pw, Color: "#374151"));

            p.Text("Key Metrics", margin, 640, 12, new TinyPdfCreate.TextOptions(Font: TinyPdfCreate.PdfFont.Courier));
            ...
            p.Text("0", axisX - 18, axisBottom - 6, 8, new TinyPdfCreate.TextOptions(Align: "right", Width: 12, Color: "#374151"));
            p.Line(axisX - 4, axisTop, axisX + 4, axisTop, "#d1d5db", 1);
            p.Text(axisMax.ToString("0"), axisX - 18, axisTop - 6, 8, new TinyPdfCreate.TextOptions(Align: "right", Width: 12, Color: "#374151"));
            ...
            for (int i = 0; i < bars; i++)
            {
                ...
                p.Text(barDisplay[i].ToString("0"), x, y + h + 6, 9, new TinyPdfCreate.TextOptions(Width: barW, Align: "center", Color: "#374151"));

                // label under bar area
                p.Text(barLabels[i], x, barAreaY + 6, 9, new TinyPdfCreate.TextOptions(Width: barW, Align: "center", Color: "#374151"));
            }
            ...
            p.Text("100", lineAxisLeft - 18, lineAxisTop - 6, 8, new TinyPdfCreate.TextOptions(Align: "right", Width: 12, Color: "#374151"));
            p.Text("0", lineAxisLeft - 18, lineAxisBottom - 6, 8, new TinyPdfCreate.TextOptions(Align: "right", Width: 12, Color: "#374151"));
            ...
            for (int i = 0; i < points; i++)
            {
                ...
                prevX = px; prevY = py;
                // x label
                p.Text(("M" + (i + 1)), px - 8, lineAreaY + 6, 8, new TinyPdfCreate.TextOptions(Color: "#374151"));
            }

            // Add a metrics table under charts
            double tableY = chartTop - chartHeight - 30;
            double ty = tableY - 20;
            p.Text("Key Metrics Details", margin, ty, 12, new TinyPdfCreate.TextOptions(Font: TinyPdfCreate.PdfFont.Courier));
            ty -= 18;
            ...
            foreach (var m in metrics)
            {
                ...
                p.Text(m.Item2, margin + 220, ty, 11, new TinyPdfCreate.TextOptions(Align: "right", Width: 120));
                p.Text(m.Item3, margin + 360, ty, 11, new TinyPdfCreate.TextOptions(Align: "right", Width: 120));
                ty -= 18;
            }

            // Notes (fill remaining space)
            double notesY = ty - 10;
            p.Text("Notes", margin, notesY, 12, new TinyPdfCreate.TextOptions(Font: TinyPdfCreate.PdfFont.Courier));
            notesY -= 16;
